...
<template>
    <li v-for="item in  ingredients " :key="item.name">
        <div class="relative flex justify-between gap-x-6 px-4 py-5 hover:bg-gray-50 dark:hover:bg-gray-700 sm:px-6">
            <span v-if="showLink" class="absolute left-4 top-4 -ml-px h-full w-0.5 bg-gray-700" aria-hidden="true"></span>
            <div class="flex min-w-0 gap-x-4">
                ...
                <div class="min-w-0 flex-auto">
                    ...
                    <p class="text-sm font-semibold">On Hand: <span class="text-gray-900 dark:text-white">{{ item.on_hand }}</span></p>
                    <p class="text-sm font-semibold">Need: <span class="text-gray-900 dark:text-white">{{ item.needed }}</span></p>
                    <p class="text-sm font-semibold">Can Craft: <span class="text-gray-900 dark:text-white">{{ item.crafts }}</span></p>
                    <p class="text-sm font-semibold">Possible Recipe Crafts: <span
                            :class="[hasLowestCrafts(item, ingredients) ? 'text-red-900' : 'text-gray-900 dark:text-white']" class="">{{
            ...
            <div class="flex shrink-0 items-center gap-x-4" @click="triggerCrafting(item)">
                <div class="hidden sm:flex sm:flex-col sm:items-end">
                    <span v-if="item.ingredients">
                        <p class="text-sm leading-6 text-gray-900 dark:text-white">Ingredients</p>
                    ...
                    <span v-else>
                        <p class="mr-12 text-sm leading-6 text-gray-900 dark:text-white">{{ item.type }}</p>
                    </span>
                </div>
